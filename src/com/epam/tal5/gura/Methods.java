...
public class Methods {
	...
	public void numberWork(String[] args) {
		...
		for (int i = 0; i < res.length; i++) {
			if (res[i].length != 3)
				continue;
			if (res[i][0] != res[i][1] && res[i][0] != res[i][2]
					&& res[i][1] != res[i][2])
				r += args[i] + " ";
	...
	public void shortAndLongNum(String[] args) {
		...
		System.out
				.println("THE LONGEST -" + mas[indexMax] + " LENGHT - " + Max);
		System.out.println("THE SHORTEST -" + mas[indexMin] + " LENGHT - "
				+ Min);
	...
	public void moreThanAver(String[] args) {
		...
		for (int i = 0; i < res.length - 1; i++) {
			if (res[i].length > aver)
				System.out.println(args[i] + " - " + res[i].length);
			;
		}
	...
	public void minNumbers(String[] args) {
		...
		for (int i = 0; i < res.length - 1; i++) {
			...
			if (minnumquant < numquant) {
				minnumquant = numquant;
				result = mas[i];
			}
		}
	...
	public void varNumbers(String[] args) {
		...
		for (int i = 0; i < res.length; i++) {
			...
			for (int k = 0; k < res[i].length - 1; k++)
				for (int j = k + 1; j < res[i].length; j++)
					if (res[i][k] == res[i][j])
						t++;
